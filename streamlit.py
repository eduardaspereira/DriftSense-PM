...
st.sidebar.markdown("---")
st.sidebar.markdown("## Execução do Pipeline")

col_exp1, col_exp2 = st.sidebar.columns(2)

with col_exp1:
    if st.button("Executar Experimento", key="run_exp", width='stretch'):
        try:
            script_path = os.path.join(SCRIPT_DIR, "scripts/run_experiment.py")
            with st.spinner("A processar run_experiment.py..."):
                ...
                if result.returncode == 0:
                    st.success("[OK] Pipeline executado com sucesso.")
                    st.info("[INFO] Dados extraídos guardados em: data/processed/")
                else:
                    st.error(f"[ERRO] Falha na execução: {result.stderr[:500]}")
...
with col_exp2:
    if st.button("Atualizar Cache", key="refresh_data", width='stretch'):
        st.cache_data.clear()
        st.success("[OK] Cache limpo.")

st.sidebar.markdown("---")

contamination = st.sidebar.slider(
    "Taxa de Contaminação Limiar (%)",
    min_value=0.1,
    max_value=10.0,
    value=1.0,
    step=0.1,
...
tab1, tab2, tab3 = st.tabs(["Sumário Matricial", "Estatísticas Descritivas", "Distribuições Densidade"])
...
st.info("[INFO] Vetor de características padronizado via Z-score (StandardScaler).")
...
with col2:
    if st.button("Treinar Algoritmo", width='stretch'):
...
st.markdown("## Geração de Relatórios Académicos e Dados")
...
if hasattr(st.session_state, 'model') and st.session_state.model is not None and st.session_state.df_data is not None and 'Scenario' in st.session_state.df_data.columns:
    ...
    st.info("[INFO] Inicialize o treino para habilitar os testes estatísticos não-paramétricos de drift.")
...
<div style='text-align: center; font-family: sans-serif;'>
    <p style='font-size: 13px; color: #aaa; margin-bottom: 4px;'>
        <b>DriftSense-PM</b> | Framework Científica para Deteção de Anomalias e Monitorização de Concept Drift em Tempo Real
    </p>
