...
CAMINHO_NORMAL = os.path.join(PROCESSED_DIR, 'Time_Normal_1_098_features.csv')
CAMINHO_FALHA = os.path.join(PROCESSED_DIR, 'IR007_1_110_features.csv')

FEATURES = config['feature_engineering']['features']

DET1_PERSISTENCIA = config['detectors']['det1_error_monitoring']['persistence']
DET2_TAMANHO_JANELA = config['detectors']['det2_distribution_test']['window_size_ks']
DET2_ALPHA = config['detectors']['det2_distribution_test']['alpha_ks']
LIMITE_DISTANCIA_GATE = config['gate']['limite_distancia']
...
def main():
    ...
    ocsvm = OneClassSVM(kernel=OC_SVM_KERNEL, nu=OC_SVM_NU, gamma=OC_SVM_GAMMA)
    ocsvm.fit(X_treino)
