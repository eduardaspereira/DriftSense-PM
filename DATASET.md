...
* **D2 (Operational Drift - Regime):** Modificação do ciclo de operação. A potência do motor DC (RPM) foi aumentada de 50% (baseline) para 75%, alterando o padrão de vibração estrutural. 
* **D3 (Sensor Degradation - Noise):** Injeção matemática controlada de ruído Gaussiano sobre o sinal estabilizado, emulando o envelhecimento e a perda de calibração dos sensores (bias shift). 
