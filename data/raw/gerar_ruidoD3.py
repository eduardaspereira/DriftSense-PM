import pandas as pd
import numpy as np

colunas_corretas = ['Timestamp', 'Scenario', 'Temp', 'Hum', 'AccX', 'AccY', 'AccZ', 'Sample', 'Index']

print("A ler D0_dataset.csv")

df_d3 = pd.read_csv('D0_dataset.csv', names=colunas_corretas, header=0)


fator_de_ruido = 0.15 
np.random.seed(42) #

colunas_vibracao = ['AccX', 'AccY', 'AccZ']

for eixo in colunas_vibracao:
    
    sigma = df_d3[eixo].std() * fator_de_ruido
    ruido_gaussiano = np.random.normal(0, sigma, size=len(df_d3))
    
    # Soma o ruído e arredonda logo a 1 casa decimal
    df_d3[eixo] = (df_d3[eixo] + ruido_gaussiano).round(1)

# 4. Atualizar a Label para o modelo saber que isto é uma falha
df_d3['Scenario'] = 'D3'

# 5. Guardar o novo dataset
df_d3.to_csv('D3_dataset.csv', index=False)

print("Cenário D3 gerado com sucesso")